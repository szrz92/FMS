...
(function ($) {
    ...
    jQuery.fn.mentiony = function (dotNetObjectReference, method, options) {
        var defaults = {
            ...
            templates:          {
                ...
                listItem:         '<li class="mentiony-item" data-item-id="">' +
                                  ...
                                  '<img src="https://simpleicon.com/wp-content/uploads/user1.png">' +
                                  '</div>' +
        ...
        return this.each(function () {
            var instance = $.data(this, 'mentiony') || $.data(this, 'mentiony', new MentionsInput(dotNetObjectReference, settings));
    ...
    var MentionsInput = function (dotNetObjectReference, settings) {
         ...
         * @param e
         */
        async function onInputBoxKeyPress(e) {
        ...
        function updateDataInputData(e) {
            ...
            var doc = parser.parseFromString(elmInputBoxText, 'text/html');
            var elems = doc.getElementsByClassName("mentiony-link");
            var data = [];
            for (var i = 0; i < elems.length; i++)
            {
                try {
                    const feature = {
                        id: elems[i].getAttribute("data-item-id"),
                        userName: elems[i].textContent,
                        role: ""
                    };
                    data.push(feature);
            ...
            //var arr = Array.prototype.slice.call(da);
            elmInputBox.val(convertSpace(elmInputBoxText));
            log(elmInputBox.val(), 'elmInputBoxText : ');
            tmpEle = elmInputBox;
            dotNetObjectReference.invokeMethodAsync("mention_JSInvoked", data, jQuery(doc).text());
        }
